//v 1.1.3  
//패치노트
//체크 알고리즘 삭제(주석으로) 추후 추가 예정
//킹이 없으면 경기 끝나는 체크메이트 추가    추후 삭제 예정
//AI를 만들기 위한 준비



// V 1.1.3    2020.09.26 시작 2020.09.26 끝
//chess
...
int map_draw() {			//map 으로 체스판 draw
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 8; j++) {
			...
			case 6:
		//		if (Check == 2)						//체크 일 때 색 바꾸기 지우지 말 것
			//		setColor(4, 12);
				checkmate_B = 1;
				_setmode(_fileno(stdout), _O_U16TEXT);    //유니코 드 출력
				wprintf(L"\x265A \n");
				break;
			...
			case 60:
			//	if (Check == 1)				//체크일 때 색 바꾸기 지우기 말 것
				//	setColor(1, 12);
				checkmate_W = 1;
				_setmode(_fileno(stdout), _O_U16TEXT);
				wprintf(L"\x2654 \n");
